...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR AppName[] = TEXT("Aggregation Client");
	...
	if (FAILED(hr)) {
		...
		MessageBox(NULL, TEXT("Failed To Initialize"), TEXT("CoInitailize ERROR"), MB_OK);
		exit(0);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE:
		...
		if (FAILED(hr)) {
			MessageBox(hwnd, TEXT("ISum Interface Cannot Be Found"), TEXT("ERROR for ISUM"), MB_OK);
			DestroyWindow(hwnd);
		}
		...
		if (FAILED(hr)) {
			MessageBox(hwnd, TEXT("ISubtract Interface Cannot Be Found"), TEXT("ERROR for ISuntract"), MB_OK);
			DestroyWindow(hwnd);
		}
		pISum->Release();
		pISum = NULL;
		...
		if (FAILED(hr)) {
			MessageBox(hwnd, TEXT("IMultiplication Interface Cannot Be Found"), TEXT("ERROR for IMultiplication"), MB_OK);
			DestroyWindow(hwnd);
		}
		...
		if (FAILED(hr)) {
			MessageBox(hwnd, TEXT("IDivision Interface Cannot Be Found"), TEXT("ERROR for IDivision"), MB_OK);
			DestroyWindow(hwnd);
		}
