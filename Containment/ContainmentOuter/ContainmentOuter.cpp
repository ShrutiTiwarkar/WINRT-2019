...
HRESULT CSumSubtract::InitializeInnerComponent(void) {
	...
	if (FAILED(hr)) {
		MessageBox(NULL, TEXT("IMultiplication Interface Cannot Be Found"), TEXT("Error"), MB_OK);
		return(E_FAIL);
	}
	...
	if (FAILED(hr)) {
		MessageBox(NULL, TEXT("IDivision Interface Cannot Be Found"), TEXT("Error"), MB_OK);
		return(E_FAIL);
	}
